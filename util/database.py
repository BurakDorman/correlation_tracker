...
def initialize_db(db_path):
    ...
    if db_path == "DATABASE_USER":
        ...
        for cmd in commands:
            c.execute('''INSERT INTO logs_cmd (command, usage_alltime, usage_thisweek, usage_lasttime) VALUES (?, ?, ?, ?)''', (cmd, 0, 0, 0))

        c.execute('''CREATE TABLE IF NOT EXISTS users (order INTEGER PRIMARY KEY AUTOINCREMENT, id INTEGER NOT NULL UNIQUE)''')
...
if __name__ == '__main__':
    initialize_db(DATABASE_SYSTEM)
    initialize_db(DATABASE_USER)
